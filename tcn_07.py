...
Original file is located at
    https://colab.research.google.com/drive/1Zm-ULO8FcGoI7LDpUm0HERCy48WBUagA
...
print(f'X_TCN_train shape: {X_train.shape}, y_TCN_train shape: {y_train.shape}')

"""# TCN"""

from tcn import TCN, tcn_full_summary

# Model Training, Prediction, and Evaluation for each horizon using TCN
results_tcn = {}
predictions_tcn = {}
histories_tcn = {}  # Store the training history for each horizon
...
for h in horizons:
    ...
    history = tcn_model.fit(X_train, y_train, epochs=100, batch_size=64, validation_split=0.15, shuffle=False)
    histories_tcn[h] = history  # Save the training history
    ...
    predictions_tcn[h] = (y_test_inv.flatten(), y_pred_inv.flatten())

    # Evaluation
    def TCN_accuracy_metrics(y_true, y_pred):
    ...
    metrics = TCN_accuracy_metrics(y_test_inv, y_pred_inv)
    results_tcn[h] = metrics

# Plot loss for each horizon using LSTM
plt.figure(figsize=(15, 5))
for i, h in enumerate(horizons):
    plt.subplot(1, len(horizons), i + 1)
    plt.plot(histories_tcn[h].history['loss'], label='Training Loss')
    plt.plot(histories_tcn[h].history['val_loss'], label='Validation Loss')
...
plt.suptitle('TCN Model Loss Across Horizons', fontsize=16)
...
fig.suptitle('TCN GHI Forecasting Comparison', fontsize=17)

for i, h in enumerate(horizons):
    axs[i].plot(predictions_tcn[h][0], marker='.', label='True')
    axs[i].plot(predictions_tcn[h][1], 'r', label='Predicted')
...
print("\nTCN Comparison of Metrics Across Horizons")
for h in horizons:
    print(f"\n{h}-Hour Ahead:")
    for metric, value in results_tcn[h].items():
...
for h in horizons:
    metrics_list.append({
        'Horizon (Hours)': h,
        'R^2': results_tcn[h]['R^2'],
        'MAE': results_tcn[h]['MAE'],
        'MSE': results_tcn[h]['MSE'],
        'RMSE': results_tcn[h]['RMSE']
    })
...
horizons = list(results_tcn.keys())
rmse_values = [results_tcn[h]['RMSE'] for h in horizons]

# Create a bar graph
plt.figure(figsize=(10, 6))
bars = plt.bar(horizons, rmse_values, color='skyblue')
plt.title('TCN RMSE Comparison Across Forecasting Horizons')
...
# Add a master title for the entire figure
fig.suptitle('TCN Regression Plot', fontsize=17)
...
for i, h in enumerate(horizons):
    y_true = predictions_tcn[h][0]  # Actual GHI values
    y_pred = predictions_tcn[h][1]   # Predicted GHI values
    r2_value = results_tcn[h]['R^2'] # R² value
...
# Add the Date column
results_df['Date'] = test.index[:len(predictions_tcn[1][0])]
...
results_df['Actual GHI'] = actual_ghi_values[:len(predictions_tcn[1][0])]

# Add predictions for each horizon
for h in horizons:
    pred_col_name = f'Predicted GHI ({h}-Hour Ahead)'
    pred_values = predictions_tcn[h][1]  # Get prediction values
...
# Save to CSV
results_df.to_csv('TCN07_ghi_predictions.csv', index=False)

print("CSV file 'TCN07a_ghi_predictions.csv' has been created successfully.")

# Save to Excel
results_df.to_excel('TCN07a_ghi_predictions.xlsx', index=False)
print("Excel file 'TCN07a_ghi_predictions.xlsx' has been created successfully.")
