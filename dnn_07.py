...
Original file is located at
    https://colab.research.google.com/drive/1HG2Jt7VjJZDjLE0QjE9rrC8cKs70-DUd
...
# Model Training, Prediction, and Evaluation for each horizon using DNN
results_dnn = {}
predictions_dnn = {}
histories_dnn = {}  # Store the training history for each horizon
...
for h in horizons:
    ...
    dnn_model = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(units=100, activation='relu'),  # Additional hidden layer
        tf.keras.layers.Dropout(rate=0.5),  # Additional dropout layer
        tf.keras.layers.Dense(units=50, activation='relu'),  # Another hidden layer
    ...
    history = dnn_model.fit(X_train, y_train, epochs=100, batch_size=64, validation_split=0.15, shuffle=False)
    histories_dnn[h] = history  # Save the training history
    ...
    predictions_dnn[h] = (y_test_inv.flatten(), y_pred_inv.flatten())

    # Evaluation
    def DNN_accuracy_metrics(y_true, y_pred):
    ...
    metrics = DNN_accuracy_metrics(y_test_inv, y_pred_inv)
    results_dnn[h] = metrics

# Plot loss for each horizon using LSTM
plt.figure(figsize=(15, 5))
for i, h in enumerate(horizons):
    plt.subplot(1, len(horizons), i + 1)
    plt.plot(histories_dnn[h].history['loss'], label='Training Loss')
    plt.plot(histories_dnn[h].history['val_loss'], label='Validation Loss')
...
plt.suptitle('DNN Model Loss Across Horizons', fontsize=16)
...
fig.suptitle('DNN GHI Forecasting Comparison', fontsize=17)

for i, h in enumerate(horizons):
    axs[i].plot(predictions_dnn[h][0], marker='.', label='True')
    axs[i].plot(predictions_dnn[h][1], 'r', label='Predicted')
...
print("\nDNN Comparison of Metrics Across Horizons")
for h in horizons:
    print(f"\n{h}-Hour Ahead:")
    for metric, value in results_dnn[h].items():
...
for h in horizons:
    metrics_list.append({
        'Horizon (Hours)': h,
        'R^2': results_dnn[h]['R^2'],
        'MAE': results_dnn[h]['MAE'],
        'MSE': results_dnn[h]['MSE'],
        'RMSE': results_dnn[h]['RMSE']
    })
...
print("\nLSTM Comparison of Metrics Across Horizons")
print(metrics_table)

# Extract RMSE values and corresponding horizons
horizons = list(results_dnn.keys())
rmse_values = [results_dnn[h]['RMSE'] for h in horizons]

# Create a bar graph
plt.figure(figsize=(10, 6))
bars = plt.bar(horizons, rmse_values, color='skyblue')
plt.title('DNN RMSE Comparison Across Forecasting Horizons')
...
# Add a master title for the entire figure
fig.suptitle('DNN Regression Plot', fontsize=17)
...
for i, h in enumerate(horizons):
    y_true = predictions_dnn[h][0]  # Actual GHI values
    y_pred = predictions_dnn[h][1]   # Predicted GHI values
    r2_value = results_dnn[h]['R^2'] # R² value
...
# Add the Date column
results_df['Date'] = test.index[:len(predictions_dnn[1][0])]
...
results_df['Actual GHI'] = actual_ghi_values[:len(predictions_dnn[1][0])]

# Add predictions for each horizon
for h in horizons:
    pred_col_name = f'Predicted GHI ({h}-Hour Ahead)'
    pred_values = predictions_dnn[h][1]  # Get prediction values
...
# Save to CSV
results_df.to_csv('DNN07_ghi_predictions.csv', index=False)

print("CSV file 'DNN07a_ghi_predictions.csv' has been created successfully.")

# Save to Excel
results_df.to_excel('DNN07a_ghi_predictions.xlsx', index=False)
print("Excel file 'DNN07a_ghi_predictions.xlsx' has been created successfully.")
