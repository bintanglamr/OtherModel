...
Original file is located at
    https://colab.research.google.com/drive/1thVMwUPAZF-Vk24ooIsFTlH1RkxrUh7-
...
# Model Training, Prediction, and Evaluation for each horizon using CNN
results_cnn = {}
predictions_cnn = {}
histories_cnn = {}  # Store the training history for each horizon
...
for h in horizons:
    ...
    history = cnn_model.fit(X_train, y_train, epochs=100, batch_size=64, validation_split=0.15, shuffle=False)
    histories_cnn[h] = history  # Save the training history
    ...
    predictions_cnn[h] = (y_test_inv.flatten(), y_pred_inv.flatten())

    # Evaluation
    def CNN_accuracy_metrics(y_true, y_pred):
    ...
    metrics = CNN_accuracy_metrics(y_test_inv, y_pred_inv)
    results_cnn[h] = metrics

# Plot loss for each horizon using LSTM
plt.figure(figsize=(15, 5))
for i, h in enumerate(horizons):
    plt.subplot(1, len(horizons), i + 1)
    plt.plot(histories_cnn[h].history['loss'], label='Training Loss')
    plt.plot(histories_cnn[h].history['val_loss'], label='Validation Loss')
...
plt.suptitle('CNN Model Loss Across Horizons', fontsize=16)
...
fig.suptitle('CNN GHI Forecasting Comparison', fontsize=17)

for i, h in enumerate(horizons):
    axs[i].plot(predictions_cnn[h][0], marker='.', label='True')
    axs[i].plot(predictions_cnn[h][1], 'r', label='Predicted')
...
print("\CNN Comparison of Metrics Across Horizons")
for h in horizons:
    print(f"\n{h}-Hour Ahead:")
    for metric, value in results_cnn[h].items():
...
for h in horizons:
    metrics_list.append({
        'Horizon (Hours)': h,
        'R^2': results_cnn[h]['R^2'],
        'MAE': results_cnn[h]['MAE'],
        'MSE': results_cnn[h]['MSE'],
        'RMSE': results_cnn[h]['RMSE']
    })
...
print("\nCNN Comparison of Metrics Across Horizons")
print(metrics_table)

# Extract RMSE values and corresponding horizons
horizons = list(results_cnn.keys())
rmse_values = [results_cnn[h]['RMSE'] for h in horizons]

# Create a bar graph
plt.figure(figsize=(10, 6))
bars = plt.bar(horizons, rmse_values, color='skyblue')
plt.title('CNN RMSE Comparison Across Forecasting Horizons')
...
# Add a master title for the entire figure
fig.suptitle('CNN Regression Plot', fontsize=17)
...
for i, h in enumerate(horizons):
    y_true = predictions_cnn[h][0]  # Actual GHI values
    y_pred = predictions_cnn[h][1]   # Predicted GHI values
    r2_value = results_cnn[h]['R^2'] # R² value
...
# Add the Date column
results_df['Date'] = test.index[:len(predictions_cnn[1][0])]
...
results_df['Actual GHI'] = actual_ghi_values[:len(predictions_cnn[1][0])]

# Add predictions for each horizon
for h in horizons:
    pred_col_name = f'Predicted GHI ({h}-Hour Ahead)'
    pred_values = predictions_cnn[h][1]  # Get prediction values
...
# Save to CSV
results_df.to_csv('CNNM07a_ghi_predictions.csv', index=False)

print("CSV file 'CNN07_ghi_predictions.csv' has been created successfully.")

# Save to Excel
results_df.to_excel('CNN07a_ghi_predictions.xlsx', index=False)
print("Excel file 'GRU07a_ghi_predictions.xlsx' has been created successfully.")
