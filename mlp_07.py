...
Original file is located at
    https://colab.research.google.com/drive/1Ky5UFQyGGAYAOpm0V0Q0Ol6pQj7lQBC6
...
print(f'X_MLP_train shape: {X_train.shape}, y_MLP_train shape: {y_train.shape}')

"""# RNN"""

# Model Training, Prediction, and Evaluation for each horizon using MLP
results_mlp = {}
predictions_mlp = {}
histories_mlp = {}  # Store the training history for each horizon
...
for h in horizons:
    ...
    history = mlp_model.fit(X_train, y_train, epochs=100, batch_size=64, validation_split=0.15, shuffle=False)
    histories_mlp[h] = history  # Save the training history
    ...
    predictions_mlp[h] = (y_test_inv.flatten(), y_pred_inv.flatten())

    # Evaluation
    def MLP_accuracy_metrics(y_true, y_pred):
    ...
    metrics = MLP_accuracy_metrics(y_test_inv, y_pred_inv)
    results_mlp[h] = metrics

# Plot loss for each horizon using LSTM
plt.figure(figsize=(15, 5))
for i, h in enumerate(horizons):
    plt.subplot(1, len(horizons), i + 1)
    plt.plot(histories_mlp[h].history['loss'], label='Training Loss')
    plt.plot(histories_mlp[h].history['val_loss'], label='Validation Loss')
...
plt.suptitle('MLP Model Loss Across Horizons', fontsize=16)
...
fig.suptitle('MLP GHI Forecasting Comparison', fontsize=17)

for i, h in enumerate(horizons):
    axs[i].plot(predictions_mlp[h][0], marker='.', label='True')
    axs[i].plot(predictions_mlp[h][1], 'r', label='Predicted')
...
print("\nMLP Comparison of Metrics Across Horizons")
for h in horizons:
    print(f"\n{h}-Hour Ahead:")
    for metric, value in results_mlp[h].items():
...
for h in horizons:
    metrics_list.append({
        'Horizon (Hours)': h,
        'R^2': results_mlp[h]['R^2'],
        'MAE': results_mlp[h]['MAE'],
        'MSE': results_mlp[h]['MSE'],
        'RMSE': results_mlp[h]['RMSE']
    })
...
horizons = list(results_mlp.keys())
rmse_values = [results_mlp[h]['RMSE'] for h in horizons]

# Create a bar graph
plt.figure(figsize=(10, 6))
bars = plt.bar(horizons, rmse_values, color='skyblue')
plt.title('MLP RMSE Comparison Across Forecasting Horizons')
...
# Add a master title for the entire figure
fig.suptitle('MLP Regression Plot', fontsize=17)
...
for i, h in enumerate(horizons):
    y_true = predictions_mlp[h][0]  # Actual GHI values
    y_pred = predictions_mlp[h][1]   # Predicted GHI values
    r2_value = results_mlp[h]['R^2'] # R² value
...
# Add the Date column
results_df['Date'] = test.index[:len(predictions_mlp[1][0])]
...
results_df['Actual GHI'] = actual_ghi_values[:len(predictions_mlp[1][0])]

# Add predictions for each horizon
for h in horizons:
    pred_col_name = f'Predicted GHI ({h}-Hour Ahead)'
    pred_values = predictions_mlp[h][1]  # Get prediction values
...
# Save to CSV
results_df.to_csv('MLP07_ghi_predictions.csv', index=False)

print("CSV file 'MLP07a_ghi_predictions.csv' has been created successfully.")

# Save to Excel
results_df.to_excel('MLP07a_ghi_predictions.xlsx', index=False)
print("Excel file 'MLP07a_ghi_predictions.xlsx' has been created successfully.")
